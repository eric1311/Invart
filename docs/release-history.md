...
| v0.52 | Implemented | Local policy-sensitivity slice that varies policy/reviewer postures and reports stable critical-deny and benign-allow conclusions without treating threshold-sensitive outcomes as universal facts. |
| v0.53 | Implemented | Task-shaped installed-agent managed-wrapper slice for PATH-resolved Claude Code and Codex binaries, with deterministic benign, destructive-token, credential-exposure-token, and critical cells, direct baselines, and explicit non-claims around provider-side task solving or upstream benchmark scores. |
...
| v0.9.9 | Implemented | Live adapter conformance contract v2: each real-agent row now records evidence level, binary source, control position, side-effect timing, required artifacts, claimable coverage, and a claim gate that rejects vendor/import/discovery/missing-binary evidence inflated into Invart-mediated or enforced coverage. |
