...
from .models import Finding, Invocation, ReviewFinding, SemanticReview, TaintState, utc_now
...
class OpenAICompatibleProvider:
    ...
    def from_env(cls) -> "OpenAICompatibleProvider | None":
        api_key = os.environ.get("KAPPASKI_LLM_API_KEY") or os.environ.get("OPENAI_API_KEY")
        model = os.environ.get("KAPPASKI_LLM_MODEL")
        if not api_key or not model:
            return None
        base_url = os.environ.get("KAPPASKI_LLM_BASE_URL", "https://api.openai.com/v1")
    ...
    def complete_json(self, payload: dict[str, Any]) -> dict[str, Any]:
        request_payload = {
            ...
            "messages": [
                {"role": "system", "content": "You are Kappaski's semantic runtime reviewer. Return strict JSON only."},
...
class StaticJSONProvider:
    ...
    def from_env(cls) -> "StaticJSONProvider | None":
        raw = os.environ.get("KAPPASKI_LLM_REVIEW_JSON")
        if not raw:
            return None
        loaded = json.loads(raw)
        if not isinstance(loaded, dict):
            raise ValueError("KAPPASKI_LLM_REVIEW_JSON must be a JSON object")
        return cls(loaded)
...
class LLMReviewer:
    ...
    def __init__(self, provider: LLMReviewProvider | None = None, model: str | None = None, prompt_version: str = "llm-review.v0.2"):
        ...
        self.model = model or os.environ.get("KAPPASKI_LLM_MODEL")
    ...
    def review(self, invocation: Invocation, taint: TaintState, findings: list[Finding] | None = None) -> SemanticReview:
        ...
        response = self.provider.complete_json({
            "schema": "kappaski.semantic_review.v0.8",
