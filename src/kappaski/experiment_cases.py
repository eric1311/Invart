...
from .path_graph import export_execution_graph_json
from .postruntime import export_proof_report
from .replay import export_replay_html
from .runtime import close_session, record_action, record_outcome, start_session
from .models import RuntimeEvent, utc_now


EXPERIMENT_SCHEMA_VERSION = "kappaski.experiment_case.v0.30"
...
def list_experiment_suites() -> dict[str, Any]:
    ...
    return {
        "schema_version": "kappaski.experiment_suites.v0.39",
...
def cases_for_suite(suite: str) -> list[ExperimentCase]:
    ...
    if fixture_path.exists():
        from .experiment_fixtures import load_experiment_cases_from_file
    ...
    if suite == "external-ipi-control-plane":
        from .corpus_adapters.agentdojo import load_agentdojo_cases
        from .corpus_adapters.agentdyn import load_agentdyn_cases

        return load_agentdojo_cases() + load_agentdyn_cases()
    if suite == "authority-dataflow-boundary":
        from .corpus_adapters.agentsecbench import load_agentsecbench_cases
    ...
    if suite == "skill-supply-chain-control-plane":
        from .corpus_adapters.skill_inject import load_skill_inject_cases
...
def run_experiment_suite(suite: str, *, out_dir: Path | None = None) -> dict[str, Any]:
    ...
    if suite == "paper-ready-experiment-suite":
        return run_paper_suite(out_dir or Path(tempfile.mkdtemp(prefix="kappaski_paper_suite_")))

    root = (out_dir or Path(tempfile.mkdtemp(prefix=f"kappaski_{suite}_"))).expanduser().resolve()
    ...
    report = {
        "schema_version": "kappaski.experiment_run.v0.39",
        "suite": suite,
...
def run_experiment_case(case: ExperimentCase, out_dir: Path) -> dict[str, Any]:
    ...
    agent = bind_agent_identity("simulated-llm-agent", declared_by=principal.principal_id, adapter_agent="kappaski-experiment-harness")
    ...
    result = {
        ...
        "agent_identity": {
            ...
            "adapter_agent": "kappaski-experiment-harness",
...
def _event_from_trace_step(case: ExperimentCase, step: dict[str, Any], session_id: str, grant_id: str, index: int) -> dict[str, Any]:
    metadata = {
        ...
        "adapter": "kappaski-experiment-harness",
