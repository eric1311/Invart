...
from .path_graph import export_execution_graph_html
...
def run_adapter_command(
    *,
    ...
    registry = authority.create_session(
        ...
        metadata={"adapter_runner": "kappaski.adapter.run", "started_by": "kappaski.cli"},
    )
    ...
    if enforcement == "file-write":
        ...
        plan = authority.record_event(
            registry.session_id,
            {
                ...
                "agent": agent,
                "target": str(target),
                "metadata": {"adapter": "kappaski-adapter-run", "operation": "child_command"},
            },
            review_mode="auto",
        ...
        env = dict(os.environ)
        env["KAPPASKI_SESSION_ID"] = registry.session_id
        env["KAPPASKI_LEDGER"] = str(ledger)
        env["KAPPASKI_TARGET"] = str(target)
...
def run_adapter_runtime(
    *,
    ...
    artifacts = (out_dir or target / ".kappaski" / "adapter-runtime").expanduser().resolve()
    ...
    registry = authority.create_session(
        ...
        metadata={
            ...
            "adapter_runtime": {"kind": adapter_kind, "schema_version": "kappaski.adapter_runtime.v0.25"},
        },
    )
    ledger = Path(registry.ledger_path)
    ...
    mediated = mediate_event(
        ...
        event={
            ...
            "target": str(target),
            "metadata": {
                "adapter": f"kappaski-{adapter_kind}",
    ...
    if effect in {"allow", "audit"}:
        ...
        child_env["KAPPASKI_SESSION_ID"] = registry.session_id
        child_env["KAPPASKI_LEDGER"] = str(ledger)
        child_env["KAPPASKI_ADAPTER_KIND"] = adapter_kind
    ...
    return {
        "schema_version": "kappaski.adapter_runtime.v0.25",
        "status": status,
...
def _write_adapter_audit_json(path: Path, adapter_kind: str, proof: dict[str, Any], mediation: dict[str, Any], status: str) -> dict[str, Any]:
    payload = {
        "schema_version": "kappaski.adapter_audit.v0.25",
...
def _write_adapter_package(package_path: Path, *, adapter: dict[str, Any], artifacts: dict[str, Path]) -> Path:
    manifest = {
        "schema_version": "kappaski.adapter_package.v0.25",
        "adapter": adapter,
        "created_at": utc_now(),
        "artifacts": {
