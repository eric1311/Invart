...
def run_coverage_truthfulness_matrix(*, out_dir: Path | None = None) -> dict[str, Any]:
    root = (out_dir or Path(tempfile.mkdtemp(prefix="kappaski_coverage_matrix_"))).expanduser().resolve()
    ...
    for surface, layer in layer_by_surface.items():
        ...
        if surface == "bypass":
            ...
            coverage["degraded_reason"] = "action bypassed Kappaski mediation boundary"
    ...
    report = {
        "schema_version": "kappaski.coverage_experiments.v0.36",
