...
from .path_graph import build_execution_graph
...
def verify_proof_report(proof_path: Path | None = None, ledger_path: Path | None = None) -> dict[str, Any]:
    ...
    if proof_path is not None:
        ...
        proof = loaded
        if proof.get("schema_version") != "kappaski.proof.v0.1":
...
def summarize_session(event_log: Path) -> dict[str, Any]:
    ...
    if entries and any(entry.schema_version == "kappaski.ledger.v0.1" for entry in entries):
