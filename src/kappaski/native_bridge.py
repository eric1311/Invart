...
def render_native_response(agent: str, decision: dict[str, Any]) -> dict[str, Any]:
    ...
    reason = str(decision.get("reason") or decision.get("summary") or effect or "kappaski decision")
    allowed = effect not in {"deny", "block"}
    if agent == "claude-code":
        return {"decision": "allow" if allowed else "block", "reason": reason, "kappaski": decision}
    if agent == "codex":
        return {"allow": allowed, "message": reason, "kappaski": decision}
    if agent == "opencode":
        return {"status": "allowed" if allowed else "denied", "message": reason, "kappaski": decision}
    return {"allow": allowed, "message": reason, "kappaski": decision}
...
def bridge_conformance_matrix() -> dict[str, Any]:
    ...
    return {
        "schema_version": "kappaski.bridge_conformance.v0.16",
