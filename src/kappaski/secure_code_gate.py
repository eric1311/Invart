...
SECURE_CODE_GATE_SCHEMA_VERSION = "kappaski.secure_code_gate.v0.35"


def evaluate_secure_code_patch(case: dict[str, Any], *, out_dir: Path | None = None) -> dict[str, Any]:
    root = (out_dir or Path(tempfile.mkdtemp(prefix="kappaski_secure_code_"))).expanduser().resolve()
...
def run_secure_coding_gate_suite(*, out_dir: Path | None = None) -> dict[str, Any]:
    root = (out_dir or Path(tempfile.mkdtemp(prefix="kappaski_secure_code_suite_"))).expanduser().resolve()
    cases = [
        {
    ...
    return {
        "schema_version": "kappaski.secure_code_gate_suite.v0.35",
