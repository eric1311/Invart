...
def _result(domain: str, findings: list[dict[str, str]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "kappaski.enforcement.v0.13",
        "domain": domain,
...
def rust_shim_decision(event: dict[str, Any], *, crate_path: Path | None = None, binary_path: Path | None = None) -> dict[str, Any]:
    crate = crate_path or Path("rust/kappaski-shim")
    binary = binary_path or crate / "target" / "debug" / "kappaski-shim"
    ...
    try:
        ...
        payload = {}
    return {
        "schema_version": "kappaski.rust_shim_decision.v0.13",
...
def _rust_shim_deterministic_fallback(
    ...
    *,
    ...
    payload = {
        "schema_version": "kappaski.rust_shim_fallback.v0.13",
        "effect": guard["effect"],
...
def run_file_write_intercepted(
    ...
    *,
    ...
    event = {
        ...
        "metadata": {
            "adapter": "kappaski-rust-file-write-shim",
    ...
    result: dict[str, Any] = {
        "schema_version": "kappaski.intercepted_file_write.v0.13",
        "command": command,
        "shim_decision": shim,
        "executed": False,
        "returncode": None,
    ...
    if ledger_path is not None:
        ...
        from .runtime import record_action, record_outcome
    ...
    if effect == "deny":
        ...
        if ledger_path is not None:
            from .runtime import record_outcome

            record_outcome(ledger_path, "blocked", decision_id=decision_id, invocation_id=invocation_id, actor="kappaski-enforce", reason="rust shim denied file-write command")
        return result
    if effect == "require_approval" and require_approval_blocks:
        ...
        if ledger_path is not None:
            ...
            record_outcome(ledger_path, "blocked", decision_id=decision_id, invocation_id=invocation_id, actor="kappaski-enforce", reason="rust shim requires approval")
        return result
...
def run_enforced_command(
    ...
    *,
    ...
    result: dict[str, Any] = {
        "schema_version": "kappaski.enforced_command.v0.13",
        "domain": domain,
        "command": command,
        "guard": guard,
        "executed": False,
    ...
    if effect == "deny" or (effect == "require_approval" and require_approval_blocks):
        ...
        if ledger_path is not None:
            ...
            action, decision, _taint = record_action(
                RuntimeEvent(type="shell", session_id=session_id, command=" ".join(command), metadata={"adapter": f"kappaski-{domain}-guard", "coverage_layer": "wrapper"}),
                ledger_path,
                review_mode="off",
                policy_mode="managed",
            )
...
def rust_shim_spec(domain: str = "file-write", crate_path: Path | None = None) -> dict[str, Any]:
    ...
    return {
        "schema_version": "kappaski.rust_shim_spec.v0.13",
        "domain": domain,
        "crate": str(crate),
        "cargo_toml_exists": cargo.exists(),
        "main_rs_exists": main_rs.exists(),
        "binary_target": "kappaski-shim",
...
def rust_binary_build(crate_path: Path | None = None, *, skip_if_unavailable: bool = False) -> dict[str, Any]:
    ...
    if cargo is None:
        return {
            "schema_version": "kappaski.rust_binary_build.v0.13",
    ...
    binary = crate / "target" / "debug" / "kappaski-shim"
    return {
        "schema_version": "kappaski.rust_binary_build.v0.13",
...
def rust_build_check(crate_path: Path | None = None, *, skip_if_unavailable: bool = False) -> dict[str, Any]:
    ...
    if cargo is None:
        result = {
            "schema_version": "kappaski.rust_build_check.v0.13",
            "status": "skipped" if skip_if_unavailable else "fail",
            "cargo_available": False,
            "crate": str(crate),
            "reason": "cargo not found",
