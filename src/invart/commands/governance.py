...
from invart.surfaces.corpus import scan_corpus
from invart.governance.profiles import create_profile_distribution_bundle, load_profile_file, record_break_glass_override, resolve_profile, review_break_glass_override
from invart.governance.teamrun import append_teamrun_fact, create_blackboard_entry, create_handoff, create_teamrun, declare_agent_identity, delegate_grant, export_teamrun_aggregate, export_teamrun_proof, export_teamrun_timeline_html
from invart.governance.identity import accountability_from_ledger, bind_agent_identity, create_capability_grant, credential_inventory, declare_principal, record_identity_binding
...
def handle_daemon(args: argparse.Namespace) -> int:
    ...
    if args.daemon_command == "outcome":
        if not args.decision and not args.invocation:
            print("invart daemon outcome requires --decision or --invocation", file=sys.stderr)
            return 2
...
def handle_profile(args: argparse.Namespace) -> int:
    ...
    if args.profile_command == "registry":
        ...
        from invart.governance.profiles import create_profile_registry

        result = create_profile_registry(owner=args.owner, profiles=profiles)
        print(json.dumps(result, ensure_ascii=False, indent=2, sort_keys=True))
        return 0
    if args.profile_command == "pin":
        from invart.governance.profiles import pin_profile_bundle
    ...
    if args.profile_command == "verify":
        from invart.governance.profiles import verify_profile_bundle
