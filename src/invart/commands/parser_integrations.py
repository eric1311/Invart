...
def register_integration_commands(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    adapter_claude.add_argument("--binary", default="claude", help="Claude Code binary used for strict live evidence probing.")
    adapter_claude.add_argument("--require-live", action="store_true", help="Fail if the Claude Code binary cannot be resolved and probed.")
    ...
    real_agent_run = real_agent_sub.add_parser("run", help="Run a selected real or fixture-backed agent through an Invart managed adapter.")
    real_agent_run.add_argument("--agent", choices=adapter_profile_ids(), required=True)
    real_agent_run.add_argument("--binary", default=None, help="Agent binary override for strict live or fixture-backed validation.")
    real_agent_run.add_argument("--require-live", action="store_true")
    real_agent_run.add_argument("--target", default=".")
    real_agent_run.add_argument("--out-dir", default=".invart/real-agent-run")
    real_agent_run.add_argument("--policy-mode", choices=("audit", "advisory", "managed", "ci"), default="advisory")
    real_agent_run.add_argument("--hook-events", default=None)
    real_agent_run.add_argument("cmd", nargs=argparse.REMAINDER)
    ...
    real_agent_registry = real_agent_sub.add_parser("registry", help="Export an enterprise agent registration registry.")
    real_agent_registry.add_argument("--target", default=".")
    real_agent_registry.add_argument("--agent", action="append", choices=adapter_profile_ids(), default=[])
    real_agent_registry.add_argument("--owner", required=True)
    real_agent_registry.add_argument("--scope", choices=("session", "repo", "team"), default="repo")
    real_agent_registry.add_argument("--out", default=None)
    real_agent_registration_gate = real_agent_sub.add_parser("registration-gate", help="Check a launch request against an enterprise agent registry.")
    real_agent_registration_gate.add_argument("--registry", required=True)
    real_agent_registration_gate.add_argument("--agent", choices=adapter_profile_ids(), required=True)
    real_agent_registration_gate.add_argument("--declared-agent", default=None)
    real_agent_registration_gate.add_argument("--principal", default=None)
    real_agent_registration_gate.add_argument("--enterprise", action="store_true")
    real_agent_registration_gate.add_argument("--require-managed-launcher", action="store_true")
    real_agent_registration_gaps = real_agent_sub.add_parser("registration-gaps", help="Report discovered native agent surfaces missing enterprise registration coverage.")
    real_agent_registration_gaps.add_argument("--target", default=".")
    real_agent_registration_gaps.add_argument("--registry", required=True)
    real_agent_registration_gaps.add_argument("--include-global-config", action="store_true")
