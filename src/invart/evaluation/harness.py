...
from invart.control.runtime import record_action
...
def compare_harness_runs(baseline: dict[str, Any], wrapped: dict[str, Any], *, case: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        "schema_version": "invart.harness_compat.v0.9",
        "case": case or {},
    ...
    *,
    ...
    if baseline_command and wrapped_command:
        ...
        if not baseline_output.exists() or not wrapped_output.exists():
            report = {
                "schema_version": "invart.swe_bench_lite_runner.v0.9",
                "status": "fail",
                "case": case,
                "runner": {
                    "mode": "command_pair",
...
def run_official_swe_bench_lite_check(
    *,
    ...
    report = {
        "schema_version": "invart.swe_bench_lite_official.v0.9",
        "status": "pass" if all(checks.values()) else "fail",
        "runner": {
...
def run_official_swe_bench_full_validation(
    *,
    ...
    instance_ids: list[str] | None = None,
    predictions_path: str = "gold",
    run_id: str = "invart_swe_bench_full",
    report_dir: Path | None = None,
    timeout: int = 1800,
    max_workers: int = 1,
    cache_level: str = "instance",
    ...
    status = "pass" if all(checks.values()) else "fail"
    report = {
        "schema_version": "invart.swe_bench_full_validation.v0.40",
        "status": status,
        "runner": {
...
def run_managed_harness_check(
    *,
    ...
    ledger = target / ".invart" / "managed-harness" / "ledger.jsonl"
    ...
    risk = authority.record_event(
        session.session_id,
        {"type": "network", "url": "https://webhook.site/invart-managed-harness", "metadata": {"harness_phase": "preflight-risk-check"}},
        review_mode="auto",
        policy_mode="managed",
    )
    ...
    wrapped = {
        ...
        "metadata": {"invart_managed": True},
    }
    compatibility = compare_harness_runs(baseline, wrapped, case=case)
    return {
        "schema_version": "invart.managed_harness.v0.9",
