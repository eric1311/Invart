...
def reviewer_quality_corpus(cases: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="invart_reviewer_quality_") as tmp:
        root = Path(tmp)
    ...
    return {
        "schema_version": "invart.full_product.reviewer_quality.v0.8",
...
def optional_provider_smoke() -> dict[str, Any]:
    ...
    configured = bool(
        (os.environ.get("INVART_LLM_API_KEY") or os.environ.get("KAPPASKI_LLM_API_KEY") or os.environ.get("OPENAI_API_KEY"))
        and (os.environ.get("INVART_LLM_MODEL") or os.environ.get("KAPPASKI_LLM_MODEL"))
    )
    live_requested = os.environ.get("INVART_RUN_LIVE_PROVIDER_SMOKE") == "1" or os.environ.get("KAPPASKI_RUN_LIVE_PROVIDER_SMOKE") == "1"
    result: dict[str, Any] = {
        "schema_version": "invart.full_product.provider_smoke.v0.8",
        "provider": "openai-compatible",
        "configured": configured,
        "live_requested": live_requested,
    }
    if not configured or not live_requested:
        result.update({"status": "skipped", "reason": "set INVART_LLM_API_KEY, INVART_LLM_MODEL, and INVART_RUN_LIVE_PROVIDER_SMOKE=1 to run live smoke"})
        return result
    from invart.control.review import LLMReviewer
