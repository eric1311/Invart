...
def _render(proof: dict[str, Any], gate: dict[str, Any], case: dict[str, Any], *, include_raw: bool) -> str:
    ...
    return f"""<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Invart Replay</title><style>body{{font:14px/1.55 -apple-system,BlinkMacSystemFont,Segoe UI,Arial,sans-serif;margin:0;background:#f6f8fb;color:#172033}}header{{background:#0f172a;color:white;padding:32px 40px}}main{{max-width:1180px;margin:0 auto;padding:28px 22px}}section{{background:white;border:1px solid #dfe5ef;border-radius:12px;padding:18px;margin:16px 0}}table{{width:100%;border-collapse:collapse}}td,th{{border-bottom:1px solid #e5e7eb;padding:8px;text-align:left;vertical-align:top}}th{{background:#f1f5f9}}pre{{white-space:pre-wrap;background:#0f172a;color:#e2e8f0;padding:14px;border-radius:10px;overflow:auto}}.pill{{display:inline-block;border-radius:999px;padding:3px 8px;background:#e0ecff;color:#1d4ed8;font-weight:650}}.cards{{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:12px}}</style></head><body><header><span class='pill'>Invart Replay</span><h1>Runtime Replay Report</h1><p>Generated {esc(utc_now())}</p></header><main><section><h2>Session</h2><div class='cards'><div><b>Session</b><p>{esc(session.get('session_id'))}</p></div><div><b>Agent</b><p>{esc(session.get('agent'))}</p></div><div><b>Goal</b><p>{esc(session.get('goal'))}</p></div><div><b>Gate</b><p>{esc(gate.get('status'))}</p></div></div></section>{case_block}<section><h2>Summary</h2><pre>{esc(json.dumps(summary, ensure_ascii=False, indent=2))}</pre></section><section><h2>Coverage</h2><pre>{esc(json.dumps(proof.get('coverage', {}), ensure_ascii=False, indent=2))}</pre></section><section><h2>Timeline</h2><table><tr><th>Seq</th><th>Action</th><th>Risk</th><th>Effect</th><th>Observed</th><th>Enforced</th><th>Observed By</th><th>Enforced By</th><th>Summary</th></tr>{''.join(rows)}</table></section><section><h2>Approvals</h2><table><tr><th>Decision</th><th>Status</th><th>Approver</th><th>Reason</th></tr>{approval_rows}</table></section><section><h2>Gate Findings</h2><pre>{esc(json.dumps(gate.get('findings', []), ensure_ascii=False, indent=2))}</pre></section>{raw_block}</main></body></html>"""
