...
from invart.assurance.path_graph import build_execution_graph, query_execution_graph
...
from invart.surfaces.adapter import run_adapter_runtime, inspect_adapter_package
...
from invart.evaluation.benchmark_registry import list_benchmark_suites, optional_heavy_validation_status
...
def run_identity_principal_binding_benchmark() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="invart_v019_") as tmp:
        root = Path(tmp)
...
def run_path_graph_benchmark() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="invart_v020_") as tmp:
        root = Path(tmp)
        ledger = root / "ledger.jsonl"
...
def run_path_aware_policy_benchmark() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="invart_v021_") as tmp:
        root = Path(tmp)
        ledger = root / "ledger.jsonl"
        session = start_session(root, ledger, agent="codex", create_preflight=False)
...
def run_unified_mediation_benchmark() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="invart_v022_") as tmp:
        root = Path(tmp)
        ledger = root / "ledger.jsonl"
        session = start_session(root, ledger, agent="claude-code", create_preflight=False)
        decisions = [
        ...
        checks = {
            "schema_uniform": all(item["schema_version"] == "invart.mediation.v0.22" for item in decisions),
...
def run_enterprise_policy_governance_benchmark() -> dict[str, Any]:
    ...
    checks = {
        "registry_created": registry["schema_version"] == "invart.profile_registry.v0.23",
...
def run_pre_v1_control_plane_benchmark() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="invart_v024_") as tmp:
...
def run_adapter_runtime_integration_benchmark() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="invart_v025_") as tmp:
        root = Path(tmp)
...
def run_policy_as_code_benchmark() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="invart_v026_") as tmp:
        root = Path(tmp)
...
def run_enterprise_evidence_export_benchmark() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="invart_v027_") as tmp:
        root = Path(tmp)
        ledger = root / "ledger.jsonl"
...
def run_release_candidate_gate_benchmark() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="invart_v029_") as tmp:
        root = Path(tmp)
